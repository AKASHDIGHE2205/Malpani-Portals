...
import { getNames } from "../../../../services/post/Transaction/TransactionsAPI";
import { handlePartyNames } from "../../../../feature/post/PostEntrySlice";
...
const NameModal: FC<NameModalProps> = ({ show, setShow }) => {
  ...
  return (
    <div id="NameModal" className={`fixed inset-0 bg-gray-600 dark:bg-gray-900 bg-opacity-50 dark:bg-opacity-75 flex items-center justify-center ${show ? 'flex' : 'hidden'}`}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl dark:shadow-gray-900 max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
        <div className="p-6">
          <button
          ...
          <div className="sticky top-0 bg-white dark:bg-gray-800 pb-4">
            <div className="mb-4">
              <h1 className="text-xl font-bold text-gray-800 dark:text-white">Select Name from list...!</h1>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-100 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">Party Name</th>
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 dark:text-gray-300 uppercase tracking-wider">City Name</th>
              ...
              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                {currentItems.map((item: any, index: number) => (
                  <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{item.party_id}</td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{item.party_name}</td>
                    <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-gray-300">{item.city_name}</td>
