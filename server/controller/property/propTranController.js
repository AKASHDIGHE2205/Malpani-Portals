...
const uploadPath = path.join(__dirname, '../../uploads/property');
...
export const getActiveConsignor = (req, res) => {
  const sql = `SELECT * 
               FROM st_consignor 
               WHERE status = 'A'
               ORDER BY name ASC`;
...
export const getActiveConsignee = (req, res) => {
  const sql = `SELECT * 
               FROM st_consignee 
               WHERE status = 'A'
               ORDER BY name ASC`;
...
export const getActivePLocation = (req, res) => {
  const sql = `SELECT * 
               FROM st_ploc 
               WHERE status = 'A'
               ORDER BY name ASC`;
...
export const getActiveDocument = (req, res) => {
  const sql = `SELECT * 
               FROM st_doc 
               WHERE status = 'A'
               ORDER BY name ASC`;
...
//old Trnsaction Controller
export const getTransactionData_old = (req, res) => {
...
export const getEditPTran_old = (req, res) => {
...
//router.post("/ptran", upload.any(), createPtran);
export const newPtranOld = (req, res) => {
...
export const editPTransaction_old = (req, res) => {
...
export const deletePTransaction_old = (req, res) => {
  const { doc_no } = req.body;
...
export const newPropertyTran = (req, res) => {
  try {
...
export const getTransactionData1 = (req, res) => {
  const { from_date, to_date } = req.body;
...
export const getPtranData = (req, res) => {
...
export const updatePTran = async (req, res) => {
...
export const newDeletePTransaction = (req, res) => {
  const { doc_no } = req.body;
...
export const getSaleModalDat = (req, res) => {
...
export const getSaleProperty = (req, res) => {
...
export const newSaleProperty_old = (req, res) => {
  ...
  db.query(balanceQuery, [doc_id, doc_date, sale_survey], (balanceErr, balanceResults) => {
    ...
    if (sale_area > currentBalance) {
      return res.status(400).json({
        message: `Requested area exceeds the available sale area is ${currentBalance}. & sale area is ${sale_area}`
      });
    }
    ...
    db.query(insertSql, [doc_id, doc_date, sale_date, buyer_name, sale_value, sale_area, sale_survey, remark], (insertErr, insertResults) => {
      if (insertErr) {
      ...
      db.query(updateSql, [sale_area, doc_id, doc_date, sale_survey], (updateErr, updateResults) => {
        if (updateErr) {
...
export const newSaleProperty = (req, res) => {
  const { doc_id, doc_date, buyer_name, sale_date, sale_value, sale_area, sale_survey, remark } = req.body;
  ...
  db.query(balanceQuery, [doc_id, doc_date, sale_survey], (balanceErr, balanceResults) => {
    ...
    if (balanceResults.length === 0) {
      return res.status(404).json({ message: "No property found. Please check survey number." });
    }
    ...
    const insertSql = `
                        INSERT INTO saled_prop 
                        (doc_no, date, sale_date, buyer_name, sale_value, sale_area, sur_no, remark,status)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                      `;

    db.query(insertSql, [doc_id, doc_date, sale_date, buyer_name, sale_value, sale_area, sale_survey, remark, 'A'], (insertErr, insertResults) => {
      if (insertErr) {
        console.error("Insert Error:", insertErr);
        return res.status(500).json({ message: "Error inserting sale property" });
      }
...
export const getAllSaledProp = (req, res) => {
  const { from_date, to_date } = req.body;
...
export const updateSaledProp = (req, res) => {
  ...
  db.query(updateSql, [sale_date, buyer_name, sale_value, sale_area, sur_no, remark, sale_id, doc_no, date], (err, results) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ message: "Error updating sale property", err: err });
    }
